...
class SignatureController extends Controller
{
    ...
    public function signaturePost(Request $request)
    {
        ...
        if ($request->hasfile('inputLogopic')) 
        {
            ...
            $filename = asset('images/'.$picname);
        }else{
            $filename = asset('images/logo.png');
        }
        if ($input['inputPhone'])
        {
        ...
        }if ($input['inputPhone'] || $input['inputMobile'])
        {
            ...
            $html = '<div style="font-family:Arial; margin:60px auto; width:493px;">'. "\n" .'<img style="float:left; height:120px; width:120px;" src="' . $filename. '">' . "\n" . '<div style="margin-left:135px;">'. "\n" .'<h4 style="color:#000; font-size:18px; font-weight:500; line-height:1.15; margin:0 0 10px;"><span style="color:#05A3F4; display:block; font-size:24px; font-weight:700; padding-bottom:3px; text-transform:uppercase;">'. $input['inputFname'] .' '.  $input['inputLname'] .'</span> '.  ucwords($input['inputJobPosition']) .' at NDS Digital</h4>'. "\n" .'<h5 style="border-image:linear-gradient(35deg, #80E02B, #05A3F4) 1; border-top-style:solid; border-width:2px; color:#000; display:inline-block; font-size:15px; font-weight:500; line-height:1.5; margin:0; padding-right:30px; padding-top:12px;">'.$phone.''.$mobile .' '.$breakline.'<img style="vertical-align:middle;" src="'. asset('images/icons/email-icon.png') .'"> '. $input['inputEmail'] .' &nbsp;&nbsp;<img style="vertical-align:middle;" src="'. asset('images/icons/web-icon.png') .'"> ndsdigital.com</h5>'. "\n" .'</div>'. "\n" .'</div>';
